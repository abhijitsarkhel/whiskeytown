 ...
 *  
 */
package org.training.cockpits.setup;
...
import org.training.cockpits.constants.TrainingCockpitsConstants;
 ...
 */
@SystemSetup(extension = TrainingCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(TrainingCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(TrainingCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						TrainingCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				importImpexFile(context, "/trainingcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
			}
