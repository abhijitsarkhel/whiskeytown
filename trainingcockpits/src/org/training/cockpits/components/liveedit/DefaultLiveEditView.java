 ...
 *
 */
package org.training.cockpits.components.liveedit;
...
import org.training.cockpits.cmscockpit.session.impl.DefaultLiveEditBrowserArea;
...
public class DefaultLiveEditView extends LiveEditView<DefaultLiveEditPopupEditDialog>
{
	...
	@Override
	protected DefaultLiveEditPopupEditDialog createLiveEditPopupDialog(final String[] passedAttributes)
			throws InterruptedException
	{
		return new DefaultLiveEditPopupEditDialog(passedAttributes, getModel().getCurrentPreviewData().getCatalogVersions(),
				DefaultLiveEditView.this);
	}
	...
	public void updateItem(final TypedObject item, final Set<PropertyDescriptor> modifiedProperties, final Object reason)
	{
		...
		{
			if (this.getPopupEditorDialog() != null && this.getPopupEditorDialog().isVisible()
					&& !this.getPopupEditorDialog().equals(reason))
			{
